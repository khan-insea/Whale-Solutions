...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (!payload.title) return res.status(400).json({ success: false, error: 'Tiêu đề là bắt buộc.' });
    ...
    if (req.method === 'DELETE') {
      ...
      return res.status(200).json({ success: true, message: 'Đã xóa bài đăng thành công.' });
    }
